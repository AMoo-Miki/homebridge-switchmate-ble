...
class Scanner extends EventEmitter {
    constructor(deviceIds) {
        ...
        noble._bindings._hci.on('leScanEnableSetCmd', enabled => {
            ...
            if (enabled) {
                console.log('[SwitchmateBLE:Scanner] Scanning started');

                this.state = STATE_STARTED;

            } else {
                console.log('[SwitchmateBLE:Scanner] Scanning was terminated. Restarting momentarily...');
    ...
    start(svcIds, deviceIds) {
        ...
        if (this.deviceIds.length === 0) return console.log('[SwitchmateBLE:Scanner] Error: no devices ids have been defined.');

        this.discoveredIds = [].concat(Object.keys(this.excludedIds));
        if (this.deviceIds.length <= this.discoveredIds) {
            console.log('[SwitchmateBLE:Scanner] Delaying unnecessary scan.');
        ...
        const _onNobleOn = () => {
            if (noble.state === 'poweredOn') {
                console.log('[SwitchmateBLE:Scanner] Discovery started.');
        ...
        this._stopTimer = setTimeout(() => {
            console.log('[SwitchmateBLE:Scanner] Stopping an incomplete scan.');
            this.stop(true);
        }, this._timeout);
    }
    ...
    stop(internal) {
        ...
        if (!internal) console.log('[SwitchmateBLE:Scanner] Pausing scan...');
    ...
    onDiscover(peripheral) {
        ...
        if (version && type) {
            ...
            console.log('Discovered:', device.id, device.version, device.type);
