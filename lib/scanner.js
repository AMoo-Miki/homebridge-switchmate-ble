...
const SWITCHMATE_SERVICE_UUID_3_1 = 'a22bd383ebdd49acb2e740eb55f5d0ab';
const SWITCHMATE_SERVICE_UUID_3_2 = 'a22bd308ebdd49acb2e740eb55f5d0ab';
...
class Scanner extends EventEmitter {
    ...
    start(svcIds, deviceIds) {
        ...
        const _onNobleOn = () => {
            if (noble.state === 'poweredOn') {
                ...
                noble.startScanning(Array.isArray(svcIds) ? svcIds : [SWITCHMATE_SERVICE_UUID_1, SWITCHMATE_SERVICE_UUID_3_1, SWITCHMATE_SERVICE_UUID_3_2], true);
            }
        };
        ...
        setTimeout(() => {
            if (!nobleStarted) {
                ...
                console.log('Looks like we don\'t have access to use the Bluetooth interface.');
                this.stop();
            }
        }, 5000);
    }
    ...
    onDiscover(peripheral) {
        ...
        const manufacturerHex = manufacturerData && manufacturerData.toString('hex');

        let version, type;

        if ((serviceUuids.includes(SWITCHMATE_SERVICE_UUID_3_1) || serviceUuids.includes(SWITCHMATE_SERVICE_UUID_3_2)) &&
            manufacturerHex
        ) {
            version = 3;
            type = {4: Scanner.SWITCH, 6: Scanner.SWITCH, 8: Scanner.OUTLET}[manufacturerData.length];
            if (/^0201/.test(manufacturerHex)) type = Scanner.OUTLET;
        } else if (
