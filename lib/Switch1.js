    ...
    connect(callback) {
        ...
        const _callback = err => {
            ...
            if (err) this.device.disconnect();
                ...
                this.platform.log('[SwitchmateBLE:Switch:1] Connected to %s (%d ms)', this.device.id, Date.now() - _start);
            }
            callback(err);
        };
        ...
        async.retry({times: 5, interval: 500}, callback => {
            ...
            async.series({
                ...
                if (err && this._queue.length() === 0) {
                    if (err === 'Error:ConnectTimeout') return callback(err);
            ...
            if (err) this.platform.log('[SwitchmateBLE:Switch:1] Error on %s:', this.device.id, err);
