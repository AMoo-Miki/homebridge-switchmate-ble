...
class Switch1 extends EventEmitter {
    constructor(platform, device) {
        ...
        this._state = device._peripheral.advertisement.serviceData[0].data[4] % 2 === 1;
    ...
    connect(callback) {
        ...
        this.platform.log('Connecting to %s', this.device.id, this.device._config.authCode);
        async.series({
            Setup: next => {
    ...
    _setState(_state, callback) {
        async.retry({times: 5, interval: 500}, callback => {
                    ...
                    characteristic.notify(true, err => {
                        ...
                        characteristic.once('data', data => {
                            if (data.equals(RESET_SEQUENCE)) {
                                this.platform.log('Switch %s has reset itself. A new authCode has to be obtained by re-pairing it.', this.device.id);
            ...
            if (err) this.platform.log('Error', err);
