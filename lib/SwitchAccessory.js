...
class SwitchAccessory {
    ...
    _registerCharacteristics() {
        ...
        this.characteristicOn = service.getCharacteristic(Characteristic.On)
            .updateValue(Boolean(this.devices.reduce((result, device) => result ^ device.state, null)))
            .on('get', this.getState.bind(this))
            .on('set', this.setState.bind(this));
    }

    _updateState() {
        this.characteristicOn.updateValue(Boolean(this.devices.reduce((result, device) => result ^ device.state, null)));
    }
    ...
    getState(callback) {
        callback(null, Boolean(this.devices.reduce((result, device) => result ^ device.state, null)));
    }
