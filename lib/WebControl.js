            ...
            if (req.method === 'GET') {
                ...
                if ((match = req.url.match(URL_MATCHER)) !== null && this.platform.cachedAccessories.has(match[1])) {
                    ...
                    const _setState = (state) => {
                        switch (match[2]) {
                            case '0':
                                device.setLight(state, _next);
                                break;

                            case '1':
                                device.setState(1, state, _next);
                                break;

                            case '2':
                                device.setState(2, state, _next);
                                break;

                            case undefined:
                                device?.setState?.(state, _next);
                                break;
                    ...
                    // Use strings "on" and "off" from the tail of the request
                    const lowerCommand = match[3]?.toLowerCase?.();
                    if (lowerCommand === 'on') return _setState(true);
                    if (lowerCommand === 'off') return _setState(false);

                    // Use JSON data sent in
                    let body = '';
                    req.on('data', data => {
                        body += data;
                    });
                    req.on('end', () => {
                        try {
                            ...
                            if (typeof msg.state === 'boolean' && _setState(msg.state)) return;
