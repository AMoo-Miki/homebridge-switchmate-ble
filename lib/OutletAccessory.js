...
class OutletAccessory {
    ...
    constructor(...props) {
        ...
        this.accessory.on('identify', (paired, callback) => {
            this.platform.log("[SwitchmateBLE:Outlet] %s - identify", this.context.name);
            callback();
        });
        ...
        scanner.on('discover', device => {
            ...
            this.platform.log('[SwitchmateBLE:Outlet] Found %s (%s)', this.device.id, this._time);
            ...
            this.connect(err => {
                if (!err) this._getLight(err => {
                    if (this.characteristicLight) {
                        this.characteristicLight.updateValue(err ? new Error('Unreachable') : this._light);
                    }
                });
            });
        });
    ...
    connect(callback) {
        ...
        if (this._busyConnecting) return callback?.('Error:BusyConnecting');

        this._busyConnecting = true;
        this.platform.log('[SwitchmateBLE:Outlet] Connecting to %s (%s)', this.device.id, this._time);
        ...
        async.series({
            ...
            Instrumentation: next => {
                if (_done) return next('Warning:ConnectedElsewhere');
                this.device.once('disconnect', () => {
                    this.platform.log('[SwitchmateBLE:Outlet] Disconnected from %s', this.device.id);

                    process.nextTick(() => {
                        this.onChange(1, new Error('Unreachable'));
                        this.onChange(2, new Error('Unreachable'));
                        this.characteristicLight.updateValue(new Error('Unreachable'));

                        this.platform.log.warn('Set to unreachable: %s', this.device.id);
                ...
                this.platform.log('[SwitchmateBLE:Outlet] Connected to %s (%s)', this.device.id, this._time);
    ...
    _setPowerState(idx, _state, callback) {
        ...
        if (!characteristic) return callback('Error:PowerCharacteristic:' + idx);
    ...
    setState(idx, state, callback) {
        if (typeof(state) !== 'boolean') return process.nextTick(callback);;
